...
plot_text_div(worriedhousing_2022)+
   scale_x_continuous(expand=c(.02,.02), limits=c(.42, .73), breaks=c(.42,.54,.68,.72), labels=c("Less Worried", "Student Average", "More Worried", "Difference from \nAverage"), position="top")+
    ...
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=payhousing_2022, aes(xmin=.71, xmax=.73, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=payhousing_2022, aes(label=paste0(round(diff*100,0), "%"), x=.72, y=group_label, color=avg_above), fontface="bold", size=3)+
  ...
  scale_y_continuous(expand=c(0,0), limits=c(.4, .62), breaks=c(.4,.57), labels=c("Less Worried", "More Worried"), position="right")+
   ...
   #set psuedo grid which looks better with later formatting than the grid line
             geom_segment(data=worried_time_wide, aes(x=year, xend=year, y=.4, yend=.6),
                                  color="#b2b2b2", size=0.15)+
  labs(title="UC Comparison: I worried about my debt and financial fircumstances",
        caption=caption_ug,
