...
food_secure_time<-es%>%
  ...
  ggsave("viz/food_insecure_time.png", width=9, height=5, units="in")
...
#" \n ", scales::percent(value,accuracy=1))
 ...
 ggplot(
  data = foodsecure_2022,aes(
  y = group_label,
  x = value,
  ))+
  geom_point()+# Make a long line
  facet_grid(comp~., switch="y", scales="free_y")+
 geom_textsegment(aes(yend=group_label, xend=all, label=group_label),
                  hjust=-.01)+
