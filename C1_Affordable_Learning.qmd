...
food_secure_time<-es%>%
  ...
  scale_y_continuous(labels=scales::percent_format(accuracy=1),limits=c(.4,.5))+
...
food_secure_time_wide<-clean_wide_time(food_secure_time)
  ...
  plot_time_dumbell(food_secure_time_wide)+
   scale_y_continuous(expand=c(0,0), limits=c(.25, .56), breaks=c(.28,.5), labels=c("Less Food Insecure", "More Food Insecure"), position="right")+
             ...
             geom_segment(data=data, aes(x=year, xend=year, y=.25, yend=.5),
                                             color="#b2b2b2", size=0.15)+
    #build diff box
             #spacing of diff bar is determined on y axis
            geom_rect(data=data, aes(ymin=.5, ymax=.55, xmin=-Inf, xmax=Inf), fill="grey") +
             geom_text(data=data, aes(label=paste0(diff*100, "%"), y=.525, x=year), fontface="bold", size=3)+
            geom_text(data=filter(data, year=="2016"),
...
plot_lollipop_div(foodsecure_2022)+
  ...
  scale_y_continuous(labels=scales::percent_format(accuracy=1),limits=c(-.10,.15),
  #second axis- real value
  sec.axis = sec_axis(~ . +foodsecure_all_2022$value, labels=scales::label_percent(accuracy=1)))+
  
  #labels
   labs(title="Food Insecure",
       subtitle ="UCSD Undergrads who are food insecure",
       y=lab_diff_axis_ug,
       caption=caption_ug)
```
...
plot_text_div(payhousing_2022)+
   scale_x_continuous(expand=c(.02,.02), limits=c(.10, .34), breaks=c(.10,.2,.28,.32), labels=c("More Able to Pay", "Student Average", "More Unable to Pay", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=payhousing_2022, aes(xmin=.30, xmax=.34, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=payhousing_2022, aes(label=paste0(round(diff*100,0), "%"), x=.32, y=group_label, color=avg_above), fontface="bold", size=3)+
...
plot_lollipop_div(worriedhousing_2022)+
  ...
  #labels
   labs(title="I  worried I would not have enough \n money to cover the cost of my housing",
       subtitle ="UCSD Undergrads  worried about paying for housing",
       y=lab_diff_axis_ug,
       caption=caption_ug)
```

```{r}
plot_text_div(worriedhousing_2022)+
   scale_x_continuous(expand=c(.02,.02), limits=c(.42, .73), breaks=c(.42,.54,.68,.72), labels=c("Less Worried", "Student Average", "More Worried", "Difference from \nAverage"), position="top")+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=worriedhousing_2022, aes(xmin=.71, xmax=.73, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=worriedhousing_2022, aes(label=paste0(round(diff*100,0), "%"), x=.72, y=group_label, color=avg_above), fontface="bold", size=3)+
  labs(title="I worried I would not have enough money to cover the cost of my housing",
       subtitle ="UCSD Undergrads who were, sometimes or often",
       caption=caption_ug)
...
plot_lollipop_div(worried_2022)+
  ...
  #labels
   labs(title="I worried about my debt and financial circumstances",
       subtitle ="UCSD Undergrads worried",
       y=lab_diff_axis_ug,
       caption=caption_ug)
...
```{r}
plot_text_div(worried_2022)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.25, .62), breaks=c(.25,.42,.55,.61), labels=c("Less Worried", "Student Average", "More Worried", "Difference from \nAverage"), position="top")+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=worried_2022, aes(xmin=.60, xmax=.62, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=worried_2022, aes(label=paste0(round(diff*100,0), "%"), x=.61, y=group_label, color=avg_above), fontface="bold", size=3)+
   labs(title="I worried about my debt and financial circumstances",
        caption=caption_ug,
      subtitle = "Percent of UCSD students worried, somewhat often, often, or very often")

 ggsave("viz/worried_2022.png", width=9, height=5, units="in", dpi=300)
...
```{r}
plot_text_div(manageable_2022)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.29, .54), breaks=c(.3,.42,.49,.53), labels=c("More Managable",  "Average", "Less Manageable", "Difference from \nAverage"), position="top")+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=manageable_2022, aes(xmin=.52, xmax=.54, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=manageable_2022, aes(label=paste0(round(diff*100,0), "%"), x=.53, y=group_label, color=avg_above), fontface="bold", size=3)+
   labs(title="Given the grants and scholarships, if any, that you receive, \nthe total cost of attending UCSD is manageable",
        caption=caption_ug,
...
```{r}
plot_text_div(foodsecure_2021_gses)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.15, .41), breaks=c(.15,.21,.32,.40), labels=c("Less Food Insecure", "Student Average", "More Food Inecure", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=foodsecure_2021_gses, aes(xmin=.39, xmax=.41, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=foodsecure_2021_gses, aes(label=paste0(round(diff*100,0), "%"), x=.40, y=group_label, color=avg_above), fontface="bold", size=3)+
...
Graduate students were asked indicate their agreement with this statement "Financial Hardship has impeded my success in my program." Students who selected "Strongly Disagree," "Disagree" and "Somewhat disagree" were coded as "positive" responses.

While about 64% of students disagreed that financial hardship had impeded their success, that number drops to 48% for Hispanic/Latinx students - and First Generation, LGBT, African American, Doctoral students, and International students also have lower levels of disagreement than average.
...
```{r}
plot_text_div(hardship_2021_gses)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.25, .57), breaks=c(.25,.36,.49,.56), labels=c("Less Hardship", "Student Average", "More Hardship", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=hardship_2021_gses, aes(xmin=.55, xmax=.57, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=hardship_2021_gses, aes(label=paste0(round(diff*100,0), "%"), x=.56, y=group_label, color=avg_above), fontface="bold", size=3)+
...
```{r}
plot_text_div(concerned_2021_gses)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.56, .87), breaks=c(.57,.645,.70,.86), labels=c("Less Concern", "Student Average", "More Concern", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=concerned_2021_gses, aes(xmin=.85, xmax=.87, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=concerned_2021_gses, aes(label=paste0(round(diff*100,0), "%"), x=.86, y=group_label, color=avg_above), fontface="bold", size=3)+
  labs(title="I've been concerned about money lately.",
...
```{r}
plot_text_div(confident_2021_gses)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.20, .50), breaks=c(.2,.30,.390,.49), labels=c("More Confident", "Student Average", "Less Confident", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=confident_2021_gses, aes(xmin=.48, xmax=.50, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=confident_2021_gses, aes(label=paste0(round(diff*100,0), "%"), x=.49, y=group_label, color=avg_above), fontface="bold", size=3)+
  labs(title="I'm confident in my financial situation.",
       subtitle ="Graduate students who somewhat disagree, disagree, strongly disagree",
       caption=caption_grad)
...
```{r}
plot_text_div(getby_2021_gses)+
  scale_x_continuous(expand=c(.02,.02), limits=c(.24, .55), breaks=c(.25,.327,.49,.54), labels=c("Less cutting back", "Student Average", "More cutting back", "Difference from \nAverage"), position="top")+
    #labels=scales::percent_format(accuracy=1), )+
    scale_y_discrete(expand=c(0,1.2))+
    #build diff box
    #spacing of diff bar is determined on y axis
    geom_rect(data=getby_2021_gses, aes(xmin=.53, xmax=.55, ymin=-Inf, ymax=Inf), fill="grey") +
    geom_text(data=getby_2021_gses, aes(label=paste0(round(diff*100,0), "%"), x=.54, y=group_label, color=avg_above), fontface="bold", size=3)+
